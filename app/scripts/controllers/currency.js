...
class CurrencyController {
   ...
   * @property {number} conversionRate The conversion rate from ETC to the selected currency.
   ...
   *
   * @returns {string} The conversion rate from ETC to the selected currency.
   *
   */
  getConversionRate () {
  ...
  async updateConversionRate () {
    ...
    try {
      ...
      const response = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=ETC&tsyms=${currentCurrency.toUpperCase()}`)
      const parsedResponse = await response.json()
